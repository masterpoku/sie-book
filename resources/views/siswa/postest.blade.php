            ...
            <form action="{{ route('siswa.postest.submit', $submateri->id) }}" method="POST">
                ...
                @foreach ($quizzes as $index => $quiz)
                    <div class="mb-6">
                        ...
                        <textarea name="jawaban[{{ $quiz->id }}]" rows="4" required
                ...
                <button type="submit"
                    class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">Kirim Jawaban</button>
            </form>
        </main>

        <!-- Footer -->
        <footer class="bg-blue-600 text-white p-4 mt-8 text-center">
            <p>&copy; {{ date('Y') }} CerdasLearn. Semua Hak Dilindungi.</p>
        </footer>
    </div>
</body>
</html>
